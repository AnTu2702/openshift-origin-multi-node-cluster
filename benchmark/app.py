  ...
  for blocksize in ('512', '1k', '4k', '512k'):
     for numjobs in (1, 32, 64):
        for iodepth in (1, 8, 32, 64, 128):
           ...
           command = "fio --minimal -name=temp-fio --bs="+str(blocksize)+" --ioengine=libaio --iodepth="+str(iodepth)+" --size="+fio_size+" --direct=1 --rw="+str(run)+" --filename=/bmvol/bmfile --numjobs="+str(numjobs)+" --time_based --runtime="+fio_runtime+" --group_reporting"
           print (command)
